...
| [SerpAPI](https://serpapi.com) | API key | 100 searches/month | Google Jobs scraper; salary/location parsed from strings; posted_at in EST via `America/Toronto` |
...
```bash
cp .env.example .env
# Fill in all eight values in .env
...
│       └── 05_Skills.py         # Dark-theme skills explorer: sidebar filters, % bar chart by category
├── sql/
...
```
1. Fetch        JSearch + Adzuna + TheirStack + SerpAPI → combined list
